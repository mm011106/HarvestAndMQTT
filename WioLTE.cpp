...
/*! 
    @brief  ping コマンド発行
    @param host ipアドレスの指定（FQDNは受け付けない）
    @return True:正常終了   False: hostに対して疎通なし
	@date 2021/1/12 miyamoto
*/
bool WioLTE::Ping(const char* host){
	std::string response;
	ArgumentParser parser;

	const uint16_t pingnum=2;
	const uint16_t timeout=1; //[s]

	if (host == NULL || host[0] == '\0') return RET_ERR(false, E_UNKNOWN);
	StringBuilder str;
	// if (!str.WriteFormat("AT+QPING=1,\"%s\"", host)) return RET_ERR(-1, E_UNKNOWN);
	if (!str.WriteFormat("AT+QPING=1,\"%s\", %d, %d", host, timeout, pingnum)) return RET_ERR(false, E_UNKNOWN);

	// SerialUSB.println("WioLTE.h: hello from PING.");
	// SerialUSB.println(str.GetString());

	if (!_AtSerial.WriteCommandAndReadResponse(str.GetString(), "^OK$", 500, &response)) return RET_ERR(false, E_UNKNOWN);
	// if (!_AtSerial.WriteCommandAndReadResponse(str.GetString(), "^(+QPING: .*)$", 5000, &response)) return RET_ERR(false, E_UNKNOWN);
	// SerialUSB.println(response.c_str());
	
	// ()で囲まれた正規表現にマッチした部分を取り出して、responseに入れる
	_AtSerial.ReadResponse("^\\+QPING: ([0-9]+).*$", (timeout+1) * 1000, &response);
	const uint16_t res_code = std::stoi(response.c_str());
	// SerialUSB.println(res_code);

	switch (res_code){
		case 0:
			for (uint16_t i =0; i<pingnum; i++){
				_AtSerial.ReadResponse("^\\+QPING: ([0-9]+).*$", 500, &response);
				// SerialUSB.println(response.c_str());
			}
			// _AtSerial.ReadResponse("^\\+QPING: ([0-9]+).*$", 500, &response);
			// SerialUSB.println(response.c_str());
			...
			// SerialUSB.println("PING: success.");
		break;

		case 569:
			// SerialUSB.print("PING: No connection - ");
			// SerialUSB.println(response.c_str());
			return RET_ERR(false, E_UNKNOWN);
		break;

		case 561:
			// SerialUSB.print("PING: No carrior - ");
			// SerialUSB.println(response.c_str());
			return RET_ERR(false, E_UNKNOWN);
		break;

		default:
			// SerialUSB.print("PING:Error - ");
			// SerialUSB.println(response.c_str());
			return RET_ERR(false, E_UNKNOWN);
		break;
